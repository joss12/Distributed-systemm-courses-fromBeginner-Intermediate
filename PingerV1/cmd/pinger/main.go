...
import (
	...
	"github.com/pinger-v1/internals/pinger"
)
...
func main() {
	...
	flag.StringVar(&timeoutStr, "timeout", "1200ms", "per-request timeout (e.g 8000ms, 2)")
	flag.IntVar(&concurrency, "concurrency", 8, "max in-flight requests")
	flag.StringVar(&writeCSVPath, "cvs", "", "optional path tpo write results cvs")
	...
	if writeCSVPath != "" {
		...
		if err != nil {
			fmt.Fprintln(os.Stderr, "svc create:", err)
			os.Exit(1)
		}
		defer f.Close()
	...
	if jsonPath != "" {
		...
		fmt.Println("json write to", jsonPath)
	}
}
...
func truncate(s string, n int) string {
	...
	//rought cut that tries to preserve the right side detail (liek "i/o timeout")
	if n <= 3 {
		return s[:n]
	}
	left := n - 3
