...
import (
	...
	"github.com/pinger-v6/internal/pinger"
)
...
func main() {
	...
	flag.StringVar(&writeCSVPath, "csv", "", "optional path to write results csv")
	flag.StringVar(&jsonPath, "json", "", "write results to JSON file")
	flag.Parse()

	//if flag.NArg() == 0{
	//	fmt.Fprint(os.Stderr, "usage: %s [flags] <url1> <url2>...\n", os.Args[0])
	//	flag.PrintDefaults()
	//	os.Exit(2)
	//}
	...
	if urlsFile != "" {
		...
		if err != nil {
			fmt.Fprintln(os.Stderr, "failed to read urls-fdile:", err)
			os.Exit(2)
		}
	...
	if writeCSVPath != "" {
		...
		fmt.Println("csv written to", writeCSVPath)
	}
	if jsonPath != "" {
...
func truncate(s string, n int) string {
	...
	//rough cut that tries to preserve the right side detail
	if n <= 3 {
		return s[:n]
	}
	left := n - 3
